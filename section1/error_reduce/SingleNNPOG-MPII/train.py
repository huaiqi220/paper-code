...
'''
torchrun --nnodes=1 --nproc_per_node=4 --rdzv_id=100 --rdzv_backend=c10d --rdzv_endpoint=localhost:29402 train.py
...
def trainModel():
    ...
    save_path = os.path.join(config.save_path,config.cur_dataset,
                             str(config.batch_size) +"_" + str(config.epoch) + "_" + str(config.lr) + "_" + config.cur_k)
