...
def trainModel():
    parser = argparse.ArgumentParser(description="当前校准向量位数k")
    parser.add_argument('--calik', type=int, default=12, help="Number of cali_vector length") 
    ...
    save_path = os.path.join(config.save_path,config.cur_dataset,config.commit,
                             str(config.batch_size) +"_" + str(config.epoch) + "_" + str(config.lr)+"_" + str(cur_k))
    ...
    ddp_model = CGES.mobile_gaze_2d(config.hm_size, cur_k, 25 * 25).to(rank)
