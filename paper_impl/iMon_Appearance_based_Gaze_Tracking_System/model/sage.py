...
def conv_bn(input_x, conv, rank):
    x = conv(input_x)
    x = F.relu(x)
    # 在PyTorch中，BatchNorm层的命名是可选的，通常不需要手动命名
    bn = nn.BatchNorm2d(x.size(1)).to(rank)
    x = bn(x)
    return x

def relu_bn(input_x,rank):
    x = F.relu(input_x)
    bn = nn.BatchNorm2d(x.size(1)).to(rank)
    x = bn(x)
    return x
...
class SAGE_Imon(nn.Module):
    def __init__(self,rank, base_model='MobileNetV2', heatmap=False):
...
if __name__ == '__main__':
    ...
    m = SAGE_Imon(0,"MobileNetV2",config.heatmap).to(0)
